...
def enforce_guest_on_refresh(authenticator):
    # Streamlit refresh clears session_state but cookies can restore login automatically.
    # We allow staying logged in only immediately after a successful login submit.
    if "just_logged_in" not in st.session_state:
        st.session_state.just_logged_in = False

    # If authenticator restored a login (cookie/refresh) and we did NOT just log in this run,
    # force the app back to guest mode.
    if (
        ...
        # Best-effort cookie deletion (API differs slightly across versions)
        try:
